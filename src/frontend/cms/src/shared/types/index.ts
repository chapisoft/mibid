...
export interface SupplierPartner {
  ...
  email: string;
  phone: string;
  status: PartnerStatus | 'ACTIVE' | 'PENDING' | 'SUSPENDED';
...
export interface PartnerOnboardingRequest {
  ...
  phone: string;
  certFileName: string;
  status: PartnerOnboardingStatus | 'PENDING_APPROVAL' | 'APPROVED' | 'REJECTED' | 'NEED_CLARIFICATION';
  submittedAt: string;
  reviewNotes?: string;
}
...
export interface PartnerSupportTicket {
  ...
  partnerEmail: string;
  rfqCode: string;
  issueType: PartnerTicketIssueType | 'FORGOT_PIN' | 'EXPIRED_LINK' | 'ATTACHMENT_ERROR' | 'TECHNICAL_QUESTION';
  status: PartnerTicketStatus | 'OPEN' | 'RESOLVED' | 'IN_PROGRESS';
  requestedAt: string;
  currentPin?: string;
}
...
export interface BankRateSnapshot {
  ...
  lastSyncedAt: string;
  syncStatus: BankRateSyncStatus | 'SYNCED' | 'SYNCING' | 'ERROR';
}
...
export interface UploadedDoc {
  ...
  name: string;
  size: string;
  type: UploadedDocType | 'CO_CQ' | 'DATASHEET' | 'CATALOG' | 'OTHER';
  uploadedAt: string;
}
...
export interface ShipmentMilestone {
  ...
  plannedDate: string;
  actualDate?: string;
  status: ShipmentMilestoneStatus | 'COMPLETED' | 'IN_PROGRESS' | 'PENDING' | 'DELAYED';
  location: string;
  notes?: string;
}
...
export interface ShipmentDoc {
  id: string;
  docType: ShipmentDocType | 'ORIGINAL_BL' | 'COMMERCIAL_INVOICE' | 'PACKING_LIST' | 'CO_CERTIFICATE' | 'CQ_CERTIFICATE' | 'CUSTOMS_DECLARATION' | 'INSURANCE_POLICY';
  docName: string;
  fileSize: string;
  docNumber: string;
  isVerified: boolean;
...
export interface ShipmentItem {
  ...
  transportMode: TransportMode | 'SEA' | 'AIR' | 'ROAD' | 'RAIL';
  ...
  scheduleStatus: DeliveryScheduleStatus | 'ON_TIME' | 'EARLY' | 'DELAYED_MINOR' | 'DELAYED_CRITICAL';
...
export interface TenantAccount {
  ...
  status: TenantAccountStatus | string;
}
