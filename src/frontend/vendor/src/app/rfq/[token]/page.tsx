...
import { I18nProvider, LanguageSwitcher, useTranslation } from '../../../shared/i18n';
...
import { Currency, Incoterm, PaymentTerm, RfqLineItem, UploadedDoc } from '../../../shared/types';
...
const SAMPLE_RFQ_ITEMS: RfqLineItem[] = [
  {
    id: 'item-1',
  ...
  {
    id: 'item-2',
    itemCode: 'MC-220KV-SF6',
    itemName: 'Máy cắt điện 220kV khí SF6 loại lắp đặt ngoài trời',
    specs: 'Dòng định mức 3150A, Dòng cắt ngắn mạch 40kA/3s',
    unit: 'Bộ',
    quantity: 6,
    origin: 'ABB / Thụy Sĩ',
  },
...
function VendorPortalContent() {
  ...
  const [loadingPort, setLoadingPort] = useState('Shanghai Port, China');
  const [dischargePort, setDischargePort] = useState('Hai Phong Port, Vietnam');
  ...
  const [origins, setOrigins] = useState<Record<string, string>>({
    'item-1': 'Siemens (Đức / EU)',
    'item-2': 'ABB (Thụy Sĩ / Thụy Điển)',
  });
  const [leadTime, setLeadTime] = useState('16');
  const [warranty, setWarranty] = useState('24');
  const [notes, setNotes] = useState('Báo giá đã bao gồm chứng chỉ FAT và chuyên gia giám sát lắp đặt.');

  // Attachments state
  const [attachedFiles, setAttachedFiles] = useState<UploadedDoc[]>([
    {
      id: 'doc-1',
      name: 'ISO9001_Quality_Certificate.pdf',
      size: '2.4 MB',
      type: 'CO_CQ',
      uploadedAt: '01/09/2026 10:15',
    },
    {
      id: 'doc-2',
      name: 'Technical_Datasheet_MBA_220kV.pdf',
      size: '5.8 MB',
      type: 'DATASHEET',
      uploadedAt: '01/09/2026 10:18',
    },
  ]);
  ...
  const handleVerifyPin = () => {
    ...
    // Mã PIN hợp lệ 6 số (ví dụ: 202688 hoặc 882109 hoặc 123456)
    if (enteredPin === '202688' || enteredPin === '882109' || enteredPin === '123456') {
      setPinEntered(true);
      setAttemptsLeft(5);
      setPinErrorMsg('');
      setCaptchaInput('');
      ...
      if (nextAttempts <= 0) {
        setIsLocked(true);
        setLockoutSeconds(900); // Tạm khóa 15 phút (900 giây)
  ...
  const calculateTotal = () => {
    return SAMPLE_RFQ_ITEMS.reduce((sum, item) => {
        ...
        <div className="w-full max-w-lg rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 shadow-2xl space-y-6 text-center animate-in zoom-in-95 transition-colors">
          ...
          <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200/80 dark:border-slate-800 text-left text-xs space-y-2.5">
            <div className="flex justify-between border-b border-slate-200/60 dark:border-slate-800 pb-2">
              ...
              <span className="font-mono font-bold text-slate-900 dark:text-white">BID-2026-EVN-001 (RFQ-0881)</span>
            </div>
            <div className="flex justify-between border-b border-slate-200/60 dark:border-slate-800 pb-2">
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden space-y-0 transition-colors">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="text-xs font-mono font-bold px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full">
                {SAMPLE_RFQ_ITEMS.length} {t.portal.itemsCountLabel}
              </span>
            </div>
