...
class WorkflowService {
  ...
  public async saveTemplate(template: Partial<WorkflowTemplate>): Promise<WorkflowTemplate> {
    if (template.id && !template.id.startsWith('tpl-temp-')) {
      return await apiClient.put<WorkflowTemplate>(`/workflows/templates/${encodeURIComponent(template.id)}`, template);
    }
    return await apiClient.post<WorkflowTemplate>('/workflows/templates', template);
  }
  ...
  public async getProjectStageRequirements(projectId: string, stage?: string): Promise<any[]> {
    try {
      const res = await apiClient.get<any[]>(`/projects/${projectId}/stage-requirements?stage=${stage || 'ALL'}`);
      return Array.isArray(res) ? res : [];
    } catch {
      return [];
    }
  ...
  public async createStageRequirement(projectId: string, dto: any): Promise<any> {
    return await apiClient.post<any>(`/projects/${projectId}/stage-requirements`, dto);
  }

  public async deleteStageRequirement(itemId: string): Promise<boolean> {
    try {
      await apiClient.delete(`/projects/stage-requirements/${itemId}`);
      return true;
    } catch {
      return false;
    }
  ...
  public async toggleChecklistItemStatus(projectId: string, itemId: string, isChecked: boolean): Promise<boolean> {
    try {
      const res = await apiClient.patch<boolean>(`/projects/${projectId}/checklist-status/${itemId}?isChecked=${isChecked}`, {});
      return typeof res === 'boolean' ? res : isChecked;
    } catch {
      return isChecked;
    }
  }
