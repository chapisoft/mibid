...
export interface CreateRfqInvitationPayload {
  projectId: string;
  projectName?: string;
  projectCode?: string;
  rfqCode?: string;
...
export interface RfqInvitationResult {
  rfqId: string;
  rfqCode: string;
  projectId: string;
  projectName: string;
...
class SourcingService {
  async inviteVendor(payload: CreateRfqInvitationPayload): Promise<RfqInvitationResult> {
    return await apiClient.post<RfqInvitationResult>('/rfqs/invite', payload);
  }
    ...
    for (const rfq of projectRfqs) {
      ...
      for (const it of detail.items) {
        ...
        bomItem.quotes[rfq.supplierName] = {
          unit: uPrice,
          curr: rfq.currency,
          totalVnd: Math.round(uPrice * it.quantity * effectiveRate),
        };
      }
    }
  ...
  async verifyPortalPin(token: string, pinCode: string, captchaToken?: string): Promise<{
    success: boolean;
    errorCode?: ErrorCode;
    ...
    try {
      const res = await fetch('/api/v1/portal/verify-pin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, pin_code: pinCode, captcha_token: captchaToken }),
      });
      const json = await res.json();
      if (json && json.data) {
        return json.data;
      }
      return { success: false, errorCode: ErrorCode.BAD_REQUEST };
    } catch (err: any) {
      return { success: false, errorCode: ErrorCode.NETWORK_ERROR };
    }
  }

  async getPortalRfq(token: string): Promise<any> {
    try {
      const res = await fetch(`/api/v1/portal/rfq/${encodeURIComponent(token)}`);
      if (!res.ok) {
        return null;
      }
      const json = await res.json();
  ...
  async submitPortalQuote(token: string, quotePayload: any): Promise<any> {
    try {
      const res = await fetch(`/api/v1/portal/rfq/${encodeURIComponent(token)}/submit-quote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
