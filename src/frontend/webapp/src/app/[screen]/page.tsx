...
import { WorkflowManagementFeature } from '../../features/workflow/WorkflowManagementFeature';
...
function CmsContent() {
  ...
  const getScreenFromUrl = useCallback((): CmsScreen => {
    if (typeof window !== 'undefined') {
      ...
      // Tách biệt hoàn toàn: Khi URL là "/" hoặc "/home" -> Luôn luôn là màn hình Trang chủ (home)
    ...
    // Mặc định luôn là 'home'
    return 'home';
  }, [pathname, searchParams]);

  // Khởi tạo currentScreen ban đầu chuẩn xác theo URL hiện tại
  const [currentScreen, setCurrentScreen] = useState<CmsScreen>(getScreenFromUrl);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      ...
      if (screenFromUrl === 'login') {
        // Vào /login -> Tự động chuyển tiếp vào /dashboard
  ...
  const renderScreen = () => {
    // Màn hình Trang chủ luôn được phép hiển thị cho mọi người dùng
    ...
    switch (currentScreen) {
      ...
      case 'kanban':
        return <KanbanBoard />;
      case 'workflow':
        return <WorkflowFeature onNavigate={navigateToScreen} />;
      case 'workflows':
        return <WorkflowManagementFeature />;
      case 'sourcing':
