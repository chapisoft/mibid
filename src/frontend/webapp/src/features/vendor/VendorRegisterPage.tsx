      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/vendor"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors mr-1"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Cổng Vendor</span>
            </Link>
            <LanguageSelector />
            <ThemeToggle />
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 sm:p-10 shadow-xl space-y-6 text-center animate-in zoom-in-95">
            ...
            <div className="space-y-2">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
                {t.portal?.regSuccessTitle || 'Hồ Sơ Đăng Ký Đã Được Tiếp Nhận!'}
              </h2>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                {t.portal?.regSuccessDesc || 'Bộ phận Mua sắm MIBID sẽ thẩm định hồ sơ năng lực và phản hồi qua Email trong 24 giờ làm việc.'}
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 text-left text-xs space-y-2 max-w-md mx-auto">
              <div className="flex justify-between">
                <span className="text-slate-400">{t.portal?.companyName || 'Doanh Nghiệp'}:</span>
                <span className="font-bold text-slate-900 dark:text-white">{companyName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">{t.portal?.taxCode || 'Mã Số Thuế'}:</span>
                <span className="font-mono font-bold text-slate-900 dark:text-white">{taxCode}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">{t.portal?.contactEmail || 'Email'}:</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              <Link
                href="/vendor"
                className="inline-flex items-center justify-center gap-2 py-3 px-6 rounded-full text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/25 transition-all"
              >
                <ArrowLeft className="w-4 h-4" />
                <span>Về Cổng Nhà Cung Cấp</span>
              </Link>
              <Link
                href="/"
                className="inline-flex items-center justify-center gap-2 py-3 px-6 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-all border border-slate-200 dark:border-slate-700"
              >
                <span>Về Trang Chủ MIBID</span>
              </Link>
            </div>
          </div>
        ) : (
          <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-10 shadow-xl space-y-6 animate-in fade-in">
            <div className="space-y-1.5 border-b border-slate-100 dark:border-slate-800 pb-4">
              ...
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
                {t.portal?.regTitle || 'Đăng Ký Nhà Cung Cấp'}
              </h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t.portal?.regSubtitle || 'Gia nhập mạng lưới chuỗi cung ứng vật tư & thiết bị MIBID'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2 space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    ...
                    <span>{t.portal?.companyName || 'Tên Doanh Nghiệp / Nhà Sản Xuất'} *</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder={t.portal?.companyNamePlaceholder || 'VD: Tổng Công Ty Thiết Bị Điện Đông Anh - CTCP'}
                ...
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    ...
                    <span>{t.portal?.taxCode || 'Mã Số Thuế / Giấy Phép ĐKKD'} *</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder={t.portal?.taxCodePlaceholder || 'VD: 0100109922'}
                ...
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    ...
                    <span>{t.portal?.country || 'Quốc Gia Trụ Sở'} *</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder={t.portal?.countryPlaceholder || 'VD: Việt Nam / Nhật Bản / Đức'}
                    className="w-full h-10 px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-medium"
                  />
                </div>
                ...
                {/* Contact Person */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300">{t.portal?.contactPerson || 'Người Đại Diện Liên Hệ'} *</label>
                  <input
                    type="text"
                    required
                    value={contactPerson}
                    onChange={(e) => setContactPerson(e.target.value)}
                    placeholder={t.portal?.contactPersonPlaceholder || 'VD: Nguyễn Văn Thành (Trưởng phòng KD)'}
                    className="w-full h-10 px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-medium"
                  />
                </div>
                ...
                {/* Contact Email */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300">{t.portal?.contactEmail || 'Email Nhận Thư Mời Thầu'} *</label>
                  <input
                    type="email"
                    required
                    value={contactEmail}
                    onChange={(e) => setContactEmail(e.target.value)}
                    placeholder={t.portal?.contactEmailPlaceholder || 'kinhdoanh@eemc.com.vn'}
                    className="w-full h-10 px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-mono"
                  />
                </div>
                ...
                {/* Contact Phone */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300">{t.portal?.contactPhone || 'Số Điện Thoại / WhatsApp'} *</label>
                  <input
                    type="text"
                    required
                    value={contactPhone}
                    onChange={(e) => setContactPhone(e.target.value)}
                    placeholder={t.portal?.contactPhonePlaceholder || '+84 24 3883 2345'}
                    className="w-full h-10 px-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 font-mono"
                  />
                </div>
                ...
                {/* Supply Category */}
                <div className="space-y-1.5">
                  <label className="font-bold text-slate-700 dark:text-slate-300">{t.portal?.supplyCategory || 'Lĩnh Vực Cung Ứng Chính'}</label>
                  <input
                    type="text"
              ...
              <div className="space-y-2 pt-2">
                <label className="font-bold text-slate-700 dark:text-slate-300">{t.portal?.certUpload || 'Tải lên Giấy phép ĐKKD & Chứng chỉ Năng lực'}</label>
                <div className="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-center hover:bg-slate-50/50 dark:hover:bg-slate-950/50 transition-colors cursor-pointer relative">
                  ...
                  <p className="font-bold text-slate-900 dark:text-white">
                    {uploadedFileName || t.portal?.dragDropText || 'Kéo thả tệp tin PDF, Excel, Hình ảnh (Tối đa 25MB/tệp)'}
                  </p>
                  <p className="text-[11px] text-slate-400 mt-1">{t.portal?.certUploadHelp || 'Đính kèm giấy phép kinh doanh, chứng chỉ ISO 9001/14001'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>{t.portal?.cancelAndBack || 'Hủy & Quay Lại'}</span>
                </Link>
                ...
                >
                  ...
                  <span>{t.portal?.submitRegBtn || 'Gửi Hồ Sơ Đăng Ký'}</span>
                </button>
              </div>
            </form>
          </div>
