...
export function WorkflowDesignerPage({ workflowId, projectId, onBackToList }: WorkflowDesignerPageProps) {
  const { t } = useTranslation();
  const [workflow, setWorkflow] = useState<WorkflowDefinition | null>(null);
  const [projectInfo, setProjectInfo] = useState<{ id: string; code: string; name: string } | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      let data: WorkflowDefinition | null = null;

      // 1. Nếu có projectId, nạp dự án và quy trình được gán của dự án
      if (projectId) {
        try {
          const projectList = await tenderService.getProjects();
          const targetProj = projectList.find((p) => p.id === projectId);
          if (targetProj) {
            setProjectInfo({ id: targetProj.id, code: targetProj.projectCode, name: targetProj.projectName });
          }

          const pw = await tenderService.getProjectWorkflow(projectId);
          if (pw && pw.id && Array.isArray(pw.nodes) && pw.nodes.length > 0) {
            data = pw;
          }
        } catch (e) {
          console.warn('Lỗi nạp quy trình dự án:', e);
        }
      }

      // 2. Nếu chưa có data nhưng có workflowId, nạp theo workflowId
      if (!data && workflowId) {
        try {
          data = await workflowService.getWorkflowById(workflowId);
        } catch (e) {
          console.warn('Lỗi nạp quy trình theo ID:', e);
        }
      }

      // 3. Fallback: Nếu vẫn chưa có data hoặc nodes rỗng, tự động lấy template tiêu chuẩn đầu tiên
      if (!data || !data.nodes || data.nodes.length === 0) {
        try {
          const tpls = await workflowService.getTemplates();
          if (tpls && tpls.length > 0 && tpls[0].nodes && tpls[0].nodes.length > 0) {
            const firstTpl = tpls[0];
            data = {
              id: firstTpl.id,
              code: firstTpl.code || 'WF-STD-2026',
              name: firstTpl.name,
              version: 'v1.0',
              status: WorkflowStatus.ACTIVE,
              tenantId: '',
              tenantName: projectInfo?.name ? `Dự Án: ${projectInfo.name}` : 'Quy Trình Chuẩn Hóa',
              nodes: firstTpl.nodes,
              edges: firstTpl.edges,
              description: firstTpl.description,
              createdAt: new Date().toISOString(),
              updatedAt: new Date().toISOString(),
            };
          } else {
            data = await workflowService.getAllWorkflows().then((list) => list[0]);
          }
        } catch (e) {
          console.warn('Lỗi nạp fallback template:', e);
        }
      }
      ...
      if (data) {
        ...
        // Tự động gán liên kết quy trình với dự án nếu có projectId
        if (projectId && data.id) {
          tenderService.bindWorkflow(projectId, data.id).catch(() => {});
        }
      }
    };
    loadData();
  ...
  const handleSaveDraft = async () => {
    if (!workflow) return;
    const saved = await workflowService.saveWorkflow({ ...workflow, status: WorkflowStatus.DRAFT });
    if (projectId && saved.id) {
      await tenderService.bindWorkflow(projectId, saved.id).catch(() => {});
    }
  ...
  const handlePublish = async () => {
    ...
    if (projectId && published.id) {
      await tenderService.bindWorkflow(projectId, published.id).catch(() => {});
    }
  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        showToast('Tệp JSON không đúng định dạng!');
      }
    };
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {onBackToList && (
            <button
              ...
              className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-2xs shrink-0 cursor-pointer"
              title={projectInfo ? 'Quay lại Quản Lý Gói Thầu' : 'Quay lại Danh sách Quy trình'}
            >
          ...
          <div className="min-w-0 max-w-[280px] sm:max-w-[360px] md:max-w-[460px] lg:max-w-[560px] shrink">
            {projectInfo && (
              <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-900/60 mb-0.5 max-w-full truncate">
                <FolderKanban className="w-3 h-3 shrink-0" />
                <span className="text-[10.5px] font-bold truncate">
                  Gói Thầu: [{projectInfo.code}] {projectInfo.name}
                </span>
              </div>
            )}
            <h1
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${
                    workflow.status === WorkflowStatus.ACTIVE ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'
                  }`}
                />
                {workflow.status === WorkflowStatus.ACTIVE ? t.workflowDesigner.statusActive : t.workflowDesigner.statusDraft}
              </span>
            </div>
          </div>
