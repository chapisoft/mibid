...
import { Department, UserAccount, UserRole, UserAccountStatus } from '../../shared/types';
...
export function UserManagementPage() {
  ...
  const activeUsers = users.filter((u) => u.status !== UserAccountStatus.INACTIVE).length;
  const inactiveUsers = users.filter((u) => u.status === UserAccountStatus.INACTIVE).length;
      ...
      render: (item) => (
        <span
          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap border shadow-2xs ${
            item.status === UserAccountStatus.INACTIVE
            ...
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 space-y-1">
                ...
                <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                  {viewingUser.status === UserAccountStatus.INACTIVE ? t.common.lockedStatus : t.common.activeStatus}
                </p>
              </div>
          ...
          <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl p-6 space-y-4">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-3.5">
              ...
              <div className="space-y-1">
                ...
                <select
                  value={editingUser.status || UserAccountStatus.ACTIVE}
