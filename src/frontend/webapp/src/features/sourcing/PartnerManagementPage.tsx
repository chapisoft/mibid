...
import { SupplierPartner, PartnerOnboardingRequest, PartnerSupportTicket, PartnerStatus, PartnerTicketStatus } from '../../shared/types';
...
export function PartnerManagementPage() {
  ...
  const handleResetPin = (ticket: PartnerSupportTicket) => {
    ...
    setTickets(
      tickets.map((t) =>
        t.id === ticket.id ? { ...t, status: PartnerTicketStatus.RESOLVED, currentPin: newPin } : t
      )
    );
